...
void NocNInterface::b_transport(tlm::tlm_generic_payload& trans, sc_core::sc_time& delay) {
    ...
    // Forward transaction to external NOC using scml2::initiator_socket
...
unsigned int NocNInterface::transport_dbg(tlm::tlm_generic_payload& trans) {
    // scml2::initiator_socket implements mappable_if which has transport_dbg
...
void SmnNInterface::b_transport(tlm::tlm_generic_payload& trans, sc_core::sc_time& delay) {
    ...
    // Forward transaction to external SMN using scml2::initiator_socket
    // scml2::initiator_socket implements mappable_if which has b_transport
    initiator_socket.b_transport(trans, delay);
}
